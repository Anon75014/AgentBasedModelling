...
class Cell(ap.Agent):
    def setup(self):
        """
        Setup the cell's initial properties and initialise agent reference
        attributes.
        """
    ...
    def harvest(self):
        ...
        if self.crop == None:
            return
        self.farmer._stock[self.crop._id] += self.crop.get_harvest_yield(
            self.farmer.water_supply / self.farmer.water_need
        )
...
class BaseFarmer(ap.Agent):
    ...
    def _buy_cell(self, _coordinates):
        ...
        if self.budget <= cell.buy_cost:
            # farmer got not enough budget to buy this Cell
            return
    ...
    def change_to_crop(self, new_id: int):
        ...
        Implemented Rules:
        He can only change to a new one, if he can buy the new crop seeds for
        all his cells if the new_id is the same as the current one, onnly for
        cells with currently no crop (`crop_id == -1`) new ones will be bought
        and planted.

        :param new_id: The `crop_id` for the crop that the farmer wants to change to.
        :type new_id: int
        """
    ...
    def get_water_from_river(self) -> float:
        """
        Sets the farmer's water supply from the river.

        :return: Amount of water that the river is able to supply.
        :rtype: float
        """
