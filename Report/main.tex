...
In the next version \tagg{v2.2}, the RL agent will compete against the \href{https://n.ethz.ch/~cgolling/gess/html/code/agents.html#agents.Trader}{\texttt{Trader}} agent (see sec. \ref{sec:v13}). In this version, both spatial expansion and market decision play a role.

\begin{figure}[H]
	\includegraphics[width=\textwidth]{Report/Figures/version design.PNG}
	\centering
...
These coefficients will be used in the FAO model \cite{doi:10.1061/(ASCE)0733-9437(1991)117:5(758)} as follows :
...
\begin{figure}[H]
	\includegraphics[width=.8\textwidth]{Report/Figures/v12_Map.png}
	\centering
...
% \end{figure}
\paragraph{Commodity demand}
From \cite{zulfiqar} in agriculture the demand is usually a strong function of population to feed, and a weak function of the price.

We therefore define the global demand for crop $j$:
\begin{equation}
...
\paragraph{Commodity demand}
The supply of crop $j$ by farmer $i$ is defined as:
\begin{equation}
...
\begin{figure}[H]
    \centering
    \includegraphics[width = 1\textwidth]{Report/RL schematic.PNG}
...
\pgfplotstableread[col sep=comma]{Data/MLvsIntroBudget.csv}{\budgetc}
...
\pgfplotstableread[col sep=comma]{Data/MLvsTrader2Budget.csv}{\budgetc}

\pgfplotstableread[col sep=comma]{Data/MLvsTrader2Cellcount.csv}{\cellcountc}
...
\begin{figure}[H]
    \centering
    \includegraphics[width = 1\textwidth]{Report/Figures/stock evolution.PNG}
